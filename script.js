...
(function() {
    ...
    function renderTravelHistory(el, shipment) {
        ...
        sorted.forEach(function(entry) {
            ...
            if (entry.remark) { html += '<div class="fxg-tracking-timeline__remark">' + escapeHtml(entry.remark) + '</div>'; }
            if (entry.location) { html += '<div class="fxg-tracking-timeline__location">' + escapeHtml(entry.location) + '</div>'; }
            html += '</div></div>';
        });
        if (currentDate !== null) { html += '</div>'; }
        html += '</div>';
    ...
    window.FDX.components.push(function() {
        ...
        var F = window.FDX.admin;
        if (!trackingId) { showNotFound(el, ''); return; }
        var shipment = null;
        try { shipment = F && F.getShipment ? F.getShipment(trackingId) : null; } catch (e) { shipment = null; }
        if (!shipment) { showNotFound(el, trackingId); return; }
        try {
